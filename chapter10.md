...
* 表10-3 (表7-8と同じデータ)
* リスク差 (#oy なぜかp.252にRDの計算式が掲載)
* リスク比 (#oy RRの計算式が抜けている。邦訳版の誤植。計算結果は1.33)
...
* 交絡を測定する例：リスク比のプール化推定値と粗推定値を比較
	* 表1-1より計算される粗リスク比：0.76
	* 表1-2は交絡を示唆.
	* 式10-2を適用して計算されるリスク比：1.21


## (6) 2つ以上の変数による層化

* 本章で紹介した方法は、層化する変数が2つ以上ある場合にも、なんら修正を加えることなく適用可能。
* データを細かく分けすぎてしまうと分析が全体として正確ではなくなる。#oy 統計誤差が大きくなる。

## (7) マッチング後の層化

* コホート研究：マッチングにより交絡を防ぐことができる。#oy 表7-3参照(曝露群、非曝露群ともに男性9000人、女性1000人)。

* 症例対照研究：マッチングにより選択バイアスが生じることがある。そのような場合にはマッチング後に層化が必要。#oy 表7-5参照。症例には男性が多く、男性は曝露している人が多いような場合、性別でマッチングすると対照群も曝露者が多くなってしまい(表7-4)、層化が必要になっている(表7-5)。そもそも、マッチングせずに普通に対照群をサンプリングして、層化すればよい(第7章, p.187「対象者を選択する際に性別でマッチングしなくても、データを男女の二群に層化することによって、性別による交絡を制御することが十分可能」)。

## (8) 層化の重要性

* 多変量モデルには、複数の変数による交絡を同時に制御できるという利点がある。それにもかかわらず、分析の際にはまず層別解析を行うべきである。

* 層化には多変量解析に勝る利点がいくつかある。
	* データの分布が視覚化される。
	* 読み手も確認しやすい。
	* 分析に必要とされる前提が少ない。　
